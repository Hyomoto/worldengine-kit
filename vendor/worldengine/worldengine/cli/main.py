...
def generate_world(
    ...
    shelf_shallow=0.08,
    shelf_break=0.65,
    noisy_coastlines=0.045,
    shelf_width_noise=0.45,
    shelf_depth_noise=0.4,
    shelf_ocean_depth=1.4,
    shelf_blend=0.3,
    shelf_falloff=2.2,
    shelf_blur_steps=3,
    peak_mix=0.55,
    ...
    w = world_gen(
        ...
        elev_noise_octaves=elev_noise_octaves,
        elev_noise_amp=elev_noise_amp,
        elev_blur_steps=elev_blur_steps,
...
def main():
    ...
    g_generate.add_argument(
        ...
        default=0.06,
        metavar="N",
        help="platec orogeny strength [default=%(default)s]",
    )
    g_generate.add_argument(
        "--plate-erosion-period",
        dest="plate_erosion_period",
        type=int,
        default=90,
        metavar="N",
        help="platec tectonic erosion period (higher = more mature) [default=%(default)s]",
    )
    g_generate.add_argument(
        "--cycle-count",
        dest="cycle_count",
        ...
        default=3,
        metavar="N",
        help="platec simulation cycles [default=%(default)s]",
    )
    g_generate.add_argument(
        "--elev-noise-octaves",
        dest="elev_noise_octaves",
        type=int,
        default=4,
        metavar="N",
        help="simplex octaves added to elevation [default=%(default)s]",
    )
    g_generate.add_argument(
        "--elev-noise-amp",
        dest="elev_noise_amp",
        ...
        default=0.65,
        metavar="N",
        help="amplitude of elevation noise [default=%(default)s]",
    )
    g_generate.add_argument(
        "--elev-blur-steps",
        dest="elev_blur_steps",
        type=int,
        default=2,
        metavar="N",
        help="anti-alias passes on elevation before ocean [default=%(default)s]",
    )
    g_generate.add_argument(
        "--shelf-radius",
        dest="shelf_radius",
        ...
        default=0,
        metavar="N",
        help="continental shelf width in cells; 0 = auto from map size (~24 at 1024) [default=%(default)s]",
    )
    g_generate.add_argument(
        "--shelf-shallow",
        dest="shelf_shallow",
        ...
        default=0.08,
        metavar="N",
        help="near-coast relative depth floor [default=%(default)s]",
    )
    g_generate.add_argument(
        "--shelf-break",
        dest="shelf_break",
        ...
        default=0.65,
        metavar="N",
        help="shallow-zone bias (higher = longer soft shelf) [default=%(default)s]",
    )
    g_generate.add_argument(
        "--shelf-falloff",
        dest="shelf_falloff",
        ...
        default=2.2,
        metavar="N",
        help="soft distance→depth power (higher = gentler) [default=%(default)s]",
    )
    g_generate.add_argument(
        "--shelf-blur-steps",
        dest="shelf_blur_steps",
        type=int,
        default=3,
        metavar="N",
        help="anti-alias passes on ocean elev after shelf rewrite [default=%(default)s]",
    )
    g_generate.add_argument(
        "--noisy-coastlines",
        dest="noisy_coastlines",
        type=float,
        default=0.045,
        metavar="N",
        help="mapgen4-style coast warp near sea level [default=%(default)s]",
    )
    g_generate.add_argument(
        "--shelf-width-noise",
        dest="shelf_width_noise",
        type=float,
        default=0.45,
        metavar="N",
        help="alongshore shelf-width variation [default=%(default)s]",
    )
    g_generate.add_argument(
        "--shelf-depth-noise",
        dest="shelf_depth_noise",
        type=float,
        default=0.4,
        metavar="N",
        help="alongshore shelf-depth noise strength [default=%(default)s]",
    )
    g_generate.add_argument(
        "--shelf-ocean-depth",
        dest="shelf_ocean_depth",
        type=float,
        default=1.4,
        metavar="N",
        help="ocean depth scale (mapgen4 ocean_depth) [default=%(default)s]",
    )
    g_generate.add_argument(
        "--shelf-blend",
        dest="shelf_blend",
        ...
        default=0.3,
        metavar="N",
        help="blend shelf curve with residual platec bathymetry [default=%(default)s]",
    )
    g_generate.add_argument(
        "--peak-mix",
        dest="peak_mix",
        ...
        default=0.55,
        metavar="N",
        help="weight of plate-boundary mountain boost [default=%(default)s]",
    )
    g_generate.add_argument(
        "--peak-slope",
        dest="peak_slope",
        ...
        default=12.0,
        metavar="N",
        help="mountain distance-field falloff [default=%(default)s]",
    )
    ...
    if operation == "world":
        ...
        world = generate_world(
            ...
            plate_erosion_period=args.plate_erosion_period,
            cycle_count=args.cycle_count,
            elev_noise_octaves=args.elev_noise_octaves,
            elev_noise_amp=args.elev_noise_amp,
            elev_blur_steps=args.elev_blur_steps,
            shelf_radius=args.shelf_radius,
            shelf_shallow=args.shelf_shallow,
            shelf_break=args.shelf_break,
            noisy_coastlines=args.noisy_coastlines,
            shelf_width_noise=args.shelf_width_noise,
            shelf_depth_noise=args.shelf_depth_noise,
            shelf_ocean_depth=args.shelf_ocean_depth,
            shelf_blend=args.shelf_blend,
            shelf_falloff=args.shelf_falloff,
            shelf_blur_steps=args.shelf_blur_steps,
            peak_mix=args.peak_mix,
            peak_slope=args.peak_slope,
        )
