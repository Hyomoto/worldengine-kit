...
class PNGWriter:
    ...
    def grayscale_from_dimensions(width, height, filename=None, channel_bitdepth=16):
        return PNGWriter.from_dimensions(
            width, height, channels=1, filename=filename, channel_bitdepth=channel_bitdepth, grayscale=True
        )

    @staticmethod
    def rgb_from_dimensions(width, height, filename=None, channel_bitdepth=8):
        return PNGWriter.from_dimensions(
            width, height, channels=3, filename=filename, channel_bitdepth=channel_bitdepth
        )

    @staticmethod
    def rgba_from_dimensions(width, height, filename=None, channel_bitdepth=8):
        return PNGWriter.from_dimensions(
            width, height, channels=4, filename=filename, channel_bitdepth=channel_bitdepth, has_alpha=True
        )

    @staticmethod
    def grayscale_from_array(array, filename=None, channel_bitdepth=16, scale_to_range=False):
        return PNGWriter.from_array(
            array,
            filename=filename,
            channels=1,
    ...
    def rgb_from_array(array, filename=None, channel_bitdepth=8, scale_to_range=False):
        return PNGWriter.from_array(
            array, filename=filename, channels=3, scale_to_range=scale_to_range, channel_bitdepth=channel_bitdepth
        )
    ...
    def __init__(
        self, array, filename=None, channels=3, channel_bitdepth=8, has_alpha=False, palette=None, grayscale=False
    ):
        """
    ...
    @classmethod
    def from_dimensions(
        cls, width, height, channels, filename=None, grayscale=False, channel_bitdepth=8, has_alpha=False, palette=None
    ):
        """
        ...
        """
        assert 1 <= channels <= 4, f"PNG only supports 1 to 4 channels per pixel. Error writing {filename}."
    ...
    def complete(self, filename=None):
        ...
        if self.img is None:
            self.img = png.Writer(
                ...
                bitdepth=self.channel_bitdepth,  # British spelling
